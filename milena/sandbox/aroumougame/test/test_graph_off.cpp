...
int main()
{
  ...
  SoDB::init();
  WRL* Vrml = new WRL();
  const string& my_filespec = "test.off";

  Off off;
  off.charger(my_filespec);
    ...
    const vector <int* >& pmailles =  off.m();
			
    Lpoints3D *ptsObj = new Lpoints3D;
    for(int i=0; i< xSmt.size();i++)
      ptsObj->set1Value(i,xSmt.at(i),ySmt.at(i),zSmt.at(i));
			
    Lmailles *mailles = new Lmailles;
    for(int i=0; i< pmailles.size();i++)
    {
      if(pmailles.at(i)[0]!=3)
      {
        printf("Le maillage doit etre triangulaire\n");
        continue;
      }
      for(int j=0;j<pmailles.at(i)[0];j++)
        mailles->set1Value(i*4+j,pmailles.at(i)[j+1]);
				
      mailles->set1Value(i*4+pmailles.at(i)[0],-1);
    }
...
//       faire un reserve si hors de cette boucle
  ...
  //   trim->need_dcurv();
  shade("test.ply",400,1.5,"norm:curv:dcurv:ply_ascii:test1.ply");
...
// //           t_pair p(i,j);
// //           tmpEdge.push_back(p);
//           edge_values.push_back(max(fcurv[i], fcurv[j]));
//         }
//       }
//     }
//   }
