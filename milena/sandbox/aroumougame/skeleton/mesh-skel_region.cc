...
int main(int argc, char* argv[])
{
  ...
  typedef int curv_t;
  std::vector<uint> curvMax;
  int seuilCurvMax = 2100;
  ...
  {
    K.insert(graph_psite<point3d>(g, curvMax[i]));
  }
...
mln::graph_image< mln::point3d, bool> skeleton(pg,face_value);
using namespace mln;
p_set< graph_psite<point3d> > X;
p_set< graph_psite<point3d> > X_complement;

typedef graph_image<point3d, bool> gi_t;
mln_piter_(gi_t) p_ref(skeleton.domain());
  for_all(p_ref)
  {
    if(skeleton(p_ref))
      X.insert(p_ref);
    else
      X_complement.insert(p_ref);
  }

// K.clear();
// X = thinning(X,X_complement,2000000,false);
X = ultimateSkeleton(X,X_complement, g_ima, K, false);
  ...
  for_all(p_ref)
  {
    if(X.has(p_ref))
      ...
      skeleton(p_ref)=false;
  }

face_value = skeleton.vertex_values();
