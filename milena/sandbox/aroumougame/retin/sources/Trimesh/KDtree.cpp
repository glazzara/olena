...
KDtree::Node::Node(const float **pts, int n)
{
	...
	float dy = ymax-ymin;
	float dz = zmax-zmin;
	node.r = 0.5 * sqrt(sqr(dx) + sqr(dy) + sqr(dz));
								   ...
								   const CompatFunc *iscompat /* = NULL */) const
{
	...
	if (cache) {
		// The idea is that we keep exactly one cached point around.
		// This is used to give us an initial estimate for the closest
		// point, which hopefully prunes a large chunk of the tree away
		float myd2 = dist2(cache, k.p);
		if ((myd2 < k.closest_d2) &&
				   (!iscompat || (*iscompat)(cache))) {
			k.closest = cache;
			k.closest_d2 = myd2;
				   }
	}
		 ...
		 const CompatFunc *iscompat /* = NULL */) const
{
	...
	float one_over_dir_len = 1.0 / sqrt(sqr(dir[0])+sqr(dir[1])+sqr(dir[2]));
  ...
  if (cache) {
	  float myd2 = dist2ray2(cache, k.p, k.dir);
	  if ((myd2 < k.closest_d2) &&
			  (!iscompat || (*iscompat)(cache))) {
		  k.closest = cache;
		  k.closest_d2 = myd2;
