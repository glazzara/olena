...
/// \file tests/core/other/graph_elt_neighborhood_if.cc
///
...
int main()
{
  ...
  */
  ...
  /// Mask of vertices while iterating on vertices.
  {
    ...
    typedef graph_elt_neighborhood_if<util::graph, pv_t, v_mask_t> nbh_t;
    nbh_t nbh(v_mask);
  ...
  /// Mask of edges while iterating on edges.
  {
    ...
    typedef graph_elt_neighborhood_if<util::graph, pe_t, e_mask_t> nbh_t;
    nbh_t nbh(e_mask);
    mln_niter_(nbh_t) q(nbh, e);
    for_all(e)
      for_all(q)
	mln_assertion(q.id()%2);
  }
  ...
  /// Mask of vertices while iterating on edges.
  {
    ...
    mln_piter_(e_ima_t) e(e_ima.domain());
    typedef graph_elt_neighborhood_if<util::graph, pe_t, v_mask_t> nbh_t;
    nbh_t nbh(v_mask);
    mln_niter_(nbh_t) q(nbh, e);
    for_all(e)
      for_all(q)
